...
class Logit(BaseModel):
    def fit(self, X, y, sample_weight=None, add_constant=True):
        ...
        for _ in range(self.epochs):
            ...
            for i in range(0, X.size(0), self.batch_size):
                ...
                optimizer.zero_grad()
                y_pred = self(X_batch)
                loss = self.weighted_bce_loss(y_pred, y_batch, weight_batch)
                loss.backward()
    ...
    def weighted_bce_loss(self, y_pred, y_true, weights=None):
        if weights is None:
            return nn.BCEWithLogitsLoss()(y_pred, y_true)
        else:
            return torch.mean(
                weights
                * nn.BCEWithLogitsLoss(reduction="none")(y_pred, y_true)
            )
